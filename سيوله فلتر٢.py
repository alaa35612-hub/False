    ...
    *,
    ...
    def scan_symbol(idx: int, symbol: str):
        try:
            ...
            if not ict_setup["valid"]:
                if not SILENT_WHEN_NO_EVENTS:
                    status_label = ict_setup["ltf_stage"] if ict_setup["ltf_stage"] != "None" else "WAITING"
                    print(
                        f"Skipping {symbol}: HTF Sweep Found | LTF {status_label}",
                        flush=True,
                    )
                return idx, None, None
            ...
            status_text = "ENTRY READY" if ict_setup["valid"] else "WAITING FOR RETRACE"
            print(

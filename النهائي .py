...
class SmartMoneyAlgoProE5:
    ...
    def _collect_latest_console_events(self) -> Dict[str, Dict[str, Any]]:
        """
        تجميع الأحداث للعرض مع إصلاح مشكلة اختفاء المناطق المستمرة والمستويات.
        """
        ...
        PERSISTENT_KEYS = {
            ...
            "LIQUIDITY_LEVELS",
            "LIQUIDITY_LEVELS_4H",
            "LIQUIDITY_LEVELS_DAILY",
            "LIQUIDITY_LEVELS_MONDAY",
            "LIQUIDITY_LEVELS_MONTHLY",
        ...
        for key, value in self.console_event_log.items():
            ...
            is_persistent = any(pk in key for pk in PERSISTENT_KEYS)
            if is_persistent:
                events[key] = payload
            elif self._console_event_within_age(payload.get("time")):
                events[key] = payload

        def _text_match(label: Label, targets: List[str], exact: bool = False) -> bool:
            txt = label.text.strip()
            for t in targets:
                if exact:
                    if txt == t or txt.replace(" ", "") == t.replace(" ", ""):
                        return True
                else:
                    if t in txt:
                        return True
        ...
        def record_label(
            ...
            for lbl in reversed(self.labels):
                if not isinstance(lbl, Label):
                    continue
                if not force_persistent and not self._console_event_within_age(lbl.x):
                    continue
                if predicate(lbl):
        ...
        bull_c = self.inputs.structure.bull
        bear_c = self.inputs.structure.bear

        record_label("BOS", lambda l: _text_match(l, ["BOS", "B O S"], exact=True))
        record_label("BOS_PLUS", lambda l: _text_match(l, ["BOS+"], exact=False))
        record_label("CHOCH", lambda l: _text_match(l, [self.CHOCH_TEXT], exact=True))
        record_label("MSS_PLUS", lambda l: _text_match(l, ["MSS+"], exact=False))
        record_label("MSS", lambda l: "MSS" in l.text and "+" not in l.text)
        record_label("IDM", lambda l: _text_match(l, [self.IDM_TEXT], exact=True))
        record_label("0.5", lambda l: l.text.strip().startswith(self.MID_TEXT))

        record_label("FUTURE_BOS", lambda l: l.text.startswith(f"{self.BOS_TEXT} -"), force_persistent=True)
        record_label("FUTURE_CHOCH", lambda l: l.text.startswith(f"{self.CHOCH_TEXT} -"), force_persistent=True)

        record_label("X", lambda l: l.text.strip() == "X")

        record_label(
            "RED_CIRCLE",
            lambda l: l.style == "label.style_circle" and l.color == bear_c,
            formatter=lambda l: f"🔴 Top @ {format_price(l.y)}",
        )
        record_label(
            "GREEN_CIRCLE",
            lambda l: l.style == "label.style_circle" and l.color == bull_c,
            formatter=lambda l: f"🟢 Bottom @ {format_price(l.y)}",
        )

        def record_active_box(key: str, target_text: str) -> None:
            for bx in reversed(self.boxes):
                if bx.text.strip() == target_text:
                    events[key] = {
                        "text": bx.text,
        ...
        record_active_box("IDM_OB", "IDM OB")
        record_active_box("EXT_OB", "EXT OB")
        record_active_box("GOLDEN_ZONE", "Golden zone")
    ...
    def _apply_order_block_filters(self) -> None:
        ...
        if self.bullish_OB_Break or self.bearish_OB_Break:
            ...
            block_key = f"BLOCK_{block_status.upper()}"
            self._register_concept_event(
                block_key,
                text=f"Block {block_status.title()}",
                time_value=self.series.get_time(),
                price=price_now,
                direction=direction,
                status=block_status,
...
EVENT_DISPLAY_ORDER = [
    ...
    ("X", "Mark X"),
    ("RED_CIRCLE", "Strong High (🔴)"),
    ("GREEN_CIRCLE", "Strong Low (🟢)"),
    ("FAIR_VALUE_GAPS", "FVG General"),
    ("FAIR_VALUE_GAP_BULL", "Bullish FVG"),
    ("FAIR_VALUE_GAP_BEAR", "Bearish FVG"),
    ("LIQUIDITY_LEVELS", "Liquidity Levels"),
    ("LIQUIDITY_LEVEL_HIGH", "Liquidity High"),
    ("LIQUIDITY_LEVEL_LOW", "Liquidity Low"),
    ("GOLDEN_ZONE", "Golden Zone (OTE)"),
    ("KEY_LEVELS", "Key Levels"),
    ("KEY_LEVELS_DAILY", "Daily Level"),
    ("KEY_LEVELS_MONDAY", "Monday Level"),
    ("KEY_LEVELS_MONTHLY", "Monthly Level"),
    ("KEY_LEVELS_4H", "4H Level"),
]

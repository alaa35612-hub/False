...
class BinanceSymbolSelectorConfig:
    ...
    top_gainer_scope: Optional[str] = "15m"
    top_gainer_candle_window: Optional[int] = 50
            ...
            def scan_symbol(i: int, sym: str):
                try:
                    ...
                    if runtime is None:
                        ...
                        if candles:
                            last_processed_ts[sym] = candles[-1][0]
                    else:
                        last_ts_done = last_processed_ts.get(sym, 0)
                        # تحديث تفاضلي: عالج الشموع الجديدة فقط
                        new_candles = [c for c in candles if c[0] > last_ts_done]
                        if not new_candles:
                            return i, sym, None, None, [], None
                        runtime.process([
                            {
                                "time": c[0],
                                "open": c[1],
                        ...
                        last_processed_ts[sym] = new_candles[-1][0]
                except Exception as e:

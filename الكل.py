    ...
    *,
    ...
    def scan_symbol(idx: int, symbol: str):
        try:
            ...
            if not reasons:
                reasons.append("Market Structure Change")

            print(f"\n{_format_symbol(symbol)} ({timeframe})", flush=True)
            print(f"Status: {' | '.join(reasons)} (Last {window} bars)", flush=True)
            if liq_status["details"]:
                for detail in sorted(set(liq_status["details"])):
            ...
            summary = {
                ...
                "strategy": {
                    ...
                    "liquidity_details": sorted(set(liq_status["details"])),
                },
            }
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    target_timeframe = args.timeframe or SCANNER_TIMEFRAME
    global LIQUIDITY_SCAN_TIMEFRAMES
    LIQUIDITY_SCAN_TIMEFRAMES = {target_timeframe}

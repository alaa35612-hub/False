    ...
    *,
    ...
    def scan_symbol(idx: int, symbol: str):
        try:
            ...
            reasons: List[str] = []
            if liq_status["sweep"]:
                reasons.append(f"{ANSI_VALUE_NEG}SWEEP ⚠️{ANSI_RESET}")
            if liq_status["created"]:
                reasons.append(f"{ANSI_VALUE_POS}NEW LEVEL CREATED ✨{ANSI_RESET}")
            if liq_status["touched"]:
                reasons.append(f"{ANSI_VALUE_POS}ACTIVE TOUCH/INSIDE 🎯{ANSI_RESET}")

            print(f"\n{_format_symbol(symbol)} ({timeframe})", flush=True)
            if reasons:
                status_text = " | ".join(reasons)
                print(
                    f"{ANSI_BOLD}Status:{ANSI_RESET} {status_text} "
                    f"{ANSI_DIM}(Last {window} bars){ANSI_RESET}",
                    flush=True,
                )
            if event_lines:
            ...
            if liq_status["created_details"]:
                for detail in sorted(set(liq_status["created_details"])):
                    print(f"  -> {ANSI_VALUE_POS}Created@{ANSI_RESET} {detail}", flush=True)

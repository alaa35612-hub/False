...
# إعدادات فلترة الاستراتيجية (ICT - Michael)
    ...
    def scan_symbol(idx: int, symbol: str):
        try:
            ...
            signal_key, signal_payload, signal_time = _latest_event_payload(
                latest_events,
                ("CHOCH", "MSS", "MSS_PLUS", "BOS", "BOS_PLUS"),
            )
            ...
            if STRATEGY_REQUIRE_LIQUIDITY_SWEEP:
                liquidity_structure_match = liquidity_payload is not None and liquidity_time is not None
                if liquidity_structure_match and STRATEGY_REQUIRE_STRUCTURE:
                    liquidity_structure_match = (
                        structure_time is not None and structure_time >= liquidity_time
                    )

            close_price = runtime.series.get("close")
            retrace_match = True
            if STRATEGY_REQUIRE_RETRACE:
                retrace_match = False
            ...
            if signal_direction and sweep_direction:
                direction_alignment = signal_direction == sweep_direction

            if not (liquidity_structure_match and retrace_match and direction_alignment):
                if tracer and tracer.enabled:
                    tracer.log(
                        "scan",

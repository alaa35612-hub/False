...
def _touch_entry_within_age(runtime: Any, entry: Dict[str, Any]) -> bool:
    if runtime is None or not isinstance(entry, dict):
        return True
    try:
        if entry.get("status") not in {"touched", "retest"}:
            return True
...
def _format_telegram_entry_message(
    ...
    *,
    ...
    inside_list = [
        (label, entry)
        for label, entry in _inside_hits(hits, allowed_keys=allowed_keys)
        if _touch_entry_within_age(runtime, entry)
    ]
    if not inside_list:
        return None
    ts_display = "—"
...
def _handle_telegram_entry(symbol: str, timeframe: str, runtime: Any, hits: Dict[str, Dict[str, Any]]) -> None:
    ...
    for label, key in ZONE_LABELS:
        if allowed_keys is not None and key not in allowed_keys:
            continue
        entry = hits.get(key, {})
        if entry.get("status") == "inside" and _touch_entry_within_age(runtime, entry):

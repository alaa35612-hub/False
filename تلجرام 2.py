...
class RateLimiter:
    ...
    def __init__(self, *, max_per_sec: float = 8.0, max_concurrent: int = 50, delay_ms: int = 0,
...
def _set_rate_limits_from_cfg(cfg: Any) -> None:
    ...
    try:
        max_concurrent = int(getattr(cfg, "max_concurrent_requests", 50))
    except Exception:
        max_concurrent = 50
    try:
...
TELEGRAM_BOT_TOKEN: str = "8099231216:AAHxdhqThOi93Q0wesPByfUgIqvyvXMW2b4"
TELEGRAM_CHAT_ID: str = "119055277"
...
TOUCH_EVENT_MAX_AGE_BARS = 2
MAX_ZONE_AGE_BARS = 50
...
class ConsoleInputs:
    max_age_bars: int = 1
    touch_max_age_bars: int = TOUCH_EVENT_MAX_AGE_BARS
    zone_max_age_bars: int = MAX_ZONE_AGE_BARS
...
class SmartMoneyAlgoProE5:
    ...
    def __init__(
        ...
        if console_inputs is None:
            max_age = 1
            touch_max_age = 1
            zone_max_age = MAX_ZONE_AGE_BARS
        else:
            try:
                ...
                max_age = 1
            try:
                touch_max_age = int(getattr(console_inputs, "touch_max_age_bars", 1) or 1)
            except (TypeError, ValueError):
                touch_max_age = 1
            try:
                zone_max_age = int(getattr(console_inputs, "zone_max_age_bars", MAX_ZONE_AGE_BARS) or MAX_ZONE_AGE_BARS)
            except (TypeError, ValueError):
                zone_max_age = MAX_ZONE_AGE_BARS
        self.console_max_age_bars = max(1, max_age)
        self.touch_event_max_age_bars = max(1, touch_max_age)
        self.max_zone_age_bars = max(1, zone_max_age)
    ...
    def _touch_event_within_age(self, timestamp: Any) -> bool:
        if self.touch_event_max_age_bars <= 0:
            return True
        bars_ago = self._bars_ago_from_time(timestamp)
        if bars_ago is None:
            return True
        return bars_ago <= self.touch_event_max_age_bars
    ...
    def _collect_latest_console_events(self) -> Dict[str, Dict[str, Any]]:
        ...
        touch_keys = {"IDM_OB", "EXT_OB", "HIST_IDM_OB", "HIST_EXT_OB", "GOLDEN_ZONE"}
        for key, value in self.console_event_log.items():
            ...
            if not self._console_event_within_age(payload.get("time")):
                continue
            if key in touch_keys and payload.get("status") in {"touched", "retest"}:
                if not self._touch_event_within_age(payload.get("time")):
                    continue
            events[key] = payload
                ...
                for bx in reversed(seq):
                    ...
                    if predicate(bx):
                        if key in touch_keys:
                            current_status = events.get(key, {}).get("status")
                            current_time = events.get(key, {}).get("time", bx.left)
                            if current_status in {"touched", "retest"} and not self._touch_event_within_age(current_time):
                                return
                        events[key] = {
                            "text": bx.text,
    ...
    def _initialise_state(self) -> None:
        ...
        self._zone_expired_ids: set[int] = set()
            ...
            if bounds:
                ...
                if invalidated:
                    self._trace(
                        ...
                        "invalidate",
                        timestamp=time_val,
                        direction="up" if self.bxty == 1 else "down",
                        low=low,
                        high=high,
                        minb=minb,
                        maxb=maxb,
            ...
            for i in range(self.demandZone.size()):
                ...
                if (
                    ...
                    self._trace(
                        ...
                        top=bx.top,
                        bottom=bx.bottom,
                        reason="top>lstPrs && top<=y && bottom>=lstHlPrsIdm",
                    )
            if idx != -1:
                ...
                self._trace(
                    ...
                    idx=idx,
                    direction="up",
                    show=self.inputs.order_block.showIdmob,
                )
            ...
            for i in range(self.supplyZone.size()):
                ...
                if (
                    ...
                    self._trace(
                        ...
                        top=bx.top,
                        bottom=bx.bottom,
                        reason="bottom<lstPrs && bottom>=y && top<=lstHlPrsIdm",
                    )
            if idx != -1:
                self._archive_box(self.lstBxIdm, "Hist IDM OB", self.hist_idm_boxes)
            ...
            for i in range(self.demandZone.size()):
                ...
                if cond:
                    ...
                    self._trace(
                        ...
                        top=bx.top,
                        bottom=bx.bottom,
                        reason="top<lstPrs && top<=y && bottom>=lstHlPrs",
                    )
            if idx != -1:
                ...
                self._trace(
                    ...
                    idx=idx,
                    direction="up",
                    show=self.inputs.order_block.showExob,
                )
        else:
            idx = -1
            lstPrs = None
            for i in range(self.supplyZone.size()):
                ...
                if cond:
                    ...
                    self._trace(
                        ...
                        top=bx.top,
                        bottom=bx.bottom,
                        reason="top>lstPrs && bottom>=y && top<=lstHlPrs",
                    )
            if idx != -1:
                self._archive_box(self.lstBx, "Hist EXT OB", self.hist_ext_boxes)
...
def _resolve_index_and_time(obj: Any, series: Any) -> Tuple[Optional[int], Optional[int]]:
    ...
    if value is None:
        return None, None
    times = getattr(series, "time", []) if series is not None else []
    if not isinstance(times, list) or not times:
        return value, None if value < 0 else value
    last_index = len(times) - 1
    min_time = times[0]
    max_time = times[last_index]
    if min_time <= value <= max_time:
        idx = bisect.bisect_right(times, value) - 1
        if idx < 0:
            return None, value
        return idx, int(value)
    if 0 <= value <= last_index:
        return value, int(times[value])
    return None, int(value)


def _resolve_value_index_and_time(value: Optional[int], series: Any) -> Tuple[Optional[int], Optional[int]]:
    if value is None:
        return None, None
    times = getattr(series, "time", []) if series is not None else []
    if not isinstance(times, list) or not times:
...
def _object_creation_index(obj: Any, series: Any) -> Tuple[Optional[int], Optional[int]]:
    if isinstance(obj, Box):
        raw = getattr(obj, "left", None)
    elif isinstance(obj, Line):
        raw = getattr(obj, "x1", None)
    elif isinstance(obj, Label):
        raw = getattr(obj, "x", None)
    else:
        raw = None
    if isinstance(raw, int):
        return _resolve_value_index_and_time(raw, series)
    return None, None
...
def _is_recent(obj: Any, current_index: Optional[int], recent_bars: int, series: Any) -> bool:
    if current_index is None:
        return True
    if series is None:
        return True
    idx, _ = _resolve_index_and_time(obj, series)
    if idx is None:
        return True
...
def _box_hit_detail(
    ...
    *,
    ...
    expired_ids: Optional[set[int]] = None,
    ...
    for bx in boxes:
        if not isinstance(bx, Box):
            continue
        if expired_ids is not None and id(bx) in expired_ids:
            continue
        if max_age_bars is not None and current_index is not None:
            idx, _ = _object_creation_index(bx, series)
            if idx is not None and current_index - idx > max(0, max_age_bars - 1):
                if expired_ids is not None:
                    expired_ids.add(id(bx))
                continue
        if not _is_recent(bx, current_index, recent_bars, series):
            continue
        ...
        _, ts = _resolve_index_and_time(bx, series)
    ...
    *,
    ...
    for ln in lines:
        if not isinstance(ln, Line):
            continue
        if expired_ids is not None and id(ln) in expired_ids:
            continue
        if max_age_bars is not None and current_index is not None:
            idx, _ = _object_creation_index(ln, series)
            if idx is not None and current_index - idx > max(0, max_age_bars - 1):
                if expired_ids is not None:
                    expired_ids.add(id(ln))
                continue
        if not _is_recent(ln, current_index, recent_bars, series):
            continue
        target = float(getattr(ln, "y1", getattr(ln, "y2", math.nan)))
        if math.isnan(target):
            continue
        _, ts = _resolve_index_and_time(ln, series)
...
def _label_hit_detail(
    ...
    *,
    ...
    for lbl in labels:
        if not isinstance(lbl, Label):
            continue
        if expired_ids is not None and id(lbl) in expired_ids:
            continue
        if max_age_bars is not None and current_index is not None:
            idx, _ = _object_creation_index(lbl, series)
            if idx is not None and current_index - idx > max(0, max_age_bars - 1):
                if expired_ids is not None:
                    expired_ids.add(id(lbl))
                continue
        if not _is_recent(lbl, current_index, recent_bars, series):
            continue
        ...
        if math.isnan(target):
            continue
        _, ts = _resolve_index_and_time(lbl, series)
        if abs(price - target) <= tolerance:
    ...
    for name, container in holders:
        for bx in _iter_objects(container, Box):
            ...
            if _price_inside_box(price, bx):
                ...
                _, ts = _resolve_index_and_time(bx, series)
                return "inside", detail, True, existence_detail, ts
            distance = min(abs(price - bx.bottom), abs(price - bx.top))
            if near_limit > 0 and distance <= near_limit:
                _, ts = _resolve_index_and_time(bx, series)
                best_near = ("near", f"{name} near {format_price(bx.bottom)} → {format_price(bx.top)}", ts)
...
def check_symbol_hits(state: Any, price: Optional[float] = None) -> Dict[str, Dict[str, Any]]:
    ...
    max_zone_age_bars = int(getattr(state, "max_zone_age_bars", MAX_ZONE_AGE_BARS) or MAX_ZONE_AGE_BARS)
    max_zone_age_bars = max(1, max_zone_age_bars)
    ...
    def _box_entry(
        ...
        *,
        ...
        return _box_hit_detail(
            ...
            series=series,
            max_age_bars=max_zone_age_bars,
            expired_ids=getattr(state, "_zone_expired_ids", None),
    ...
    # Midpoint 0.5 line
    mid_line = getattr(state, "mid_line", None)
    mid_status, mid_detail, mid_ts = _line_hit_detail(
        "Midpoint 0.5",
        price_val,
        [mid_line] if isinstance(mid_line, Line) else [],
        tolerance=tolerance,
        near_limit=near_limit,
        current_index=current_index,
        recent_bars=recent_bars,
    ...
    if mid_status in {"inside", "near"} and not _touch_entry_within_age(state, {"status": "touched", "time": mid_ts}, "midpoint"):
        mid_status, mid_detail, mid_ts = "miss", None, None
    ...
    def _entry(status: str, detail: Optional[str], ts: Optional[int], extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        if status in {"inside", "near"} and isinstance(current_time, int):
            ts = current_time
        payload = {
    ...
    hits = {
        ...
        "midpoint": _entry(mid_status, mid_detail, mid_ts),
        "swing_sweep": _entry(swing_status, swing_detail, swing_ts),
        "mark_x": _entry(mark_status, mark_detail, mark_ts),
    }
    for key in ("golden_zone", "ext_ob", "idm_ob", "hist_idm_ob", "hist_ext_ob", "midpoint"):
        entry = hits.get(key)
        if not isinstance(entry, dict):
            continue
        if entry.get("status") in {"inside", "near"} and not _touch_entry_within_age(
            state,
            {"status": "touched", "time": entry.get("time")},
            key,
        ):
            entry.update({"hit": False, "status": "miss", "detail": None, "time": None})
            continue
        if entry.get("status") == "inside":
            signature = f"{key}:{entry.get('detail')}"
            prev_hits = state._zone_hit_counts.get(signature, 0)
            if prev_hits > 0:
                entry.update({"hit": False, "status": "miss", "detail": None, "time": None})
            else:
                state._zone_hit_counts[signature] = 1
    return hits
...
_TOUCH_EVENT_KEYS: Set[str] = set(_ZONE_EVENT_KEYS.values())


def _touch_entry_within_age(runtime: Any, entry: Dict[str, Any], key: Optional[str] = None) -> bool:
    if runtime is None or not isinstance(entry, dict):
        return True
    try:
        if entry.get("status") not in {"touched", "retest"} and key != "midpoint":
            return True
        timestamp = entry.get("time")
        return runtime._touch_event_within_age(timestamp)
    except Exception:
        return True
...
def _format_telegram_entry_message(
    ...
    *,
    ...
    inside_list = [
        ...
        if entry.get("status") == "inside"
        if _touch_entry_within_age(runtime, entry, key)
    ]
    if not inside_list:
        return None
    ts_display = "—"
...
def _handle_telegram_entry(symbol: str, timeframe: str, runtime: Any, hits: Dict[str, Dict[str, Any]]) -> None:
    ...
    inside_list: List[Tuple[str, Dict[str, Any]]] = []
    for label, key in ZONE_LABELS:
        if allowed_keys is not None and key not in allowed_keys:
            continue
        entry = hits.get(key, {})
        if entry.get("status") == "inside" and _touch_entry_within_age(runtime, entry, key):
            inside_list.append((label, entry))
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        help="عدد الشموع الأخيرة المستخدمة لتحديد الحداثة في نتائج المسح",
    )
    parser.add_argument(
        "--console-max-age-bars",
        type=int,
        default=1,
        help="عمر أحداث شاشة المحرر بالشموع (الحد الأدنى 1)",
    )
    parser.add_argument(
        "--touch-max-age-bars",
        type=int,
        default=TOUCH_EVENT_MAX_AGE_BARS,
        help="عمر أحداث الدخول/اللمس بالشموع (الحد الأدنى 1)",
    )
    parser.add_argument(
        "--max-zone-age-bars",
        type=int,
        default=MAX_ZONE_AGE_BARS,
        help="الحد الأقصى لعمر المنطقة بالشموع قبل تجاهلها في المسح",
    )
    parser.add_argument(
        "--continuous",
        "--continuous-scan",
    ...
    if args.console_max_age_bars <= 0:
        parser.error("--console-max-age-bars يجب أن يكون رقمًا موجبًا")
    if args.touch_max_age_bars <= 0:
        parser.error("--touch-max-age-bars يجب أن يكون رقمًا موجبًا")
    if args.max_zone_age_bars <= 0:
        parser.error("--max-zone-age-bars يجب أن يكون رقمًا موجبًا")
    if args.recent_bars <= 0:
        parser.error("--recent-bars يجب أن يكون رقمًا موجبًا")
    ...
    console_age = args.console_max_age_bars
    if args.console_max_age_bars == 1 and args.max_age_bars != 1:
        console_age = args.max_age_bars
    indicator_inputs.console.max_age_bars = console_age
    indicator_inputs.console.touch_max_age_bars = args.touch_max_age_bars
    indicator_inputs.console.zone_max_age_bars = args.max_zone_age_bars
        ...
        while True:
            ...
            runtime, summaries = asyncio.run(
                scan_binance(
                    ...
                    inputs=indicator_inputs,
                    recent_window_bars=args.recent_bars,
                )
            )
            perform_comparison()
            log("Rendering")
...
class _CLISettings:
    ...
    console_max_age_bars: int = 1
    touch_max_age_bars: int = TOUCH_EVENT_MAX_AGE_BARS
    recent_bars: int = EDITOR_AUTORUN_DEFAULTS.recent_bars
    zone_max_age_bars: int = MAX_ZONE_AGE_BARS
...
def _parse_args_android() -> Tuple[_CLISettings, argparse.Namespace]:
    ...
    p.add_argument("--recent-bars", type=int, default=EDITOR_AUTORUN_DEFAULTS.recent_bars)
    p.add_argument("--near-pct", type=float, default=EDITOR_AUTORUN_DEFAULTS.near_threshold_pct)
    p.add_argument("--console-max-age-bars", type=int, default=1)
    p.add_argument("--touch-max-age-bars", type=int, default=TOUCH_EVENT_MAX_AGE_BARS)
    p.add_argument("--max-zone-age-bars", type=int, default=MAX_ZONE_AGE_BARS)
    ...
    cfg = _CLISettings(
        ...
        console_max_age_bars=args.console_max_age_bars,
        touch_max_age_bars=args.touch_max_age_bars,
        recent_bars=args.recent_bars if getattr(args, "recent_bars", None) is not None else args.recent,
        zone_max_age_bars=args.max_zone_age_bars,
    )
    return cfg, args
...
def _should_route_android(argv: List[str]) -> bool:
    knobs = {
        ...
        ,"--recent-bars","--console-max-age-bars","--touch-max-age-bars","--max-zone-age-bars"
    }
...
def _print_ar_report(symbol, timeframe, runtime, exchange, recent_alerts):
    ...
    recent_window = max(1, cfg.recent_bars)
    ...
    inputs.console.max_age_bars = cfg.console_max_age_bars
    inputs.console.touch_max_age_bars = cfg.touch_max_age_bars
...
class Settings:
    def __init__(self, **kw):
        ...
        self.max_workers = kw.get("max_workers", 50)
        self.max_concurrent_requests = kw.get("max_concurrent_requests", 50)
...
def _android_cli_entry() -> int:
    ...
    inputs.console.max_age_bars = cfg.console_max_age_bars
    inputs.console.touch_max_age_bars = cfg.touch_max_age_bars
    inputs.console.zone_max_age_bars = cfg.zone_max_age_bars
        ...
        while True:
            ...
            for i, sym in enumerate(symbols, 1):
                try:
                    ...
                    runtime.process(candles)
                    process_elapsed = time.perf_counter() - process_started
                    # PERF logging removed to avoid scan delay
                except Exception as e:
                    print(
                        f"[{i}/{len(symbols)}] {_format_symbol(sym)}: error {e}",
                        file=sys.stderr,

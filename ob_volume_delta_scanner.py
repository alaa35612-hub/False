...
class IndicatorConfig:
    ...
    alert_on_retest_triangle: bool = True
        ...
        for ob in obs:
            ...
            if retest_prev and retest_bar is not None and retest_bar > ob.created_index:
                ...
                if can_log:
                    ob_retests.insert(
                        0,
                        RetestRec(retest_bar, retest_tm or times[retest_bar], ob.is_bull, ob.left_index, ob.created_index),
                    )
                    if ob.is_bull:
                        ev_bull_retest = True
...
def main() -> None:
    ...
    for symbol in symbols:
        try:
            ...
            if cfg.alert_on_retest_triangle and result["retests"]:
                for retest in result["retests"]:
                    side = "bull" if retest.is_bull else "bear"
                    print(
                        f"{symbol}: TRIANGLE {side} retest at {retest.bar_time} (bar {retest.bar_index})"
                    )
            if result["touches"]:

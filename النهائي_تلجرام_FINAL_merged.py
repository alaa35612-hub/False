...
EVENT_PRINT_MAX_PER_KEY = 0  # 0 = بلا حد
EVENT_PRINT_MAX_PER_FINGERPRINT = 1
...
class SmartMoneyAlgoProE5:
    ...
    def __init__(
        ...
        self.console_print_counts: Dict[str, int] = {}
        self.console_print_token_counts: Dict[str, int] = {}
        self.suppress_event_printing = False
    ...
    def _should_print_event(self, key: str, payload: Dict[str, Any]) -> bool:
        ...
        if EVENT_PRINT_KEYS and key not in EVENT_PRINT_KEYS:
            return False
        if EVENT_PRINT_MAX_PER_KEY > 0:
            if self.console_print_counts.get(key, 0) >= EVENT_PRINT_MAX_PER_KEY:
                return False
        ...
        if token:
            ...
            if last_token == token:
                return False
            if EVENT_PRINT_MAX_PER_FINGERPRINT > 0:
                if self.console_print_token_counts.get(str(token), 0) >= EVENT_PRINT_MAX_PER_FINGERPRINT:
                    return False
        return True

    def _emit_console_event(self, key: str, payload: Dict[str, Any], *, force: bool = False) -> None:
        if not isinstance(payload, dict):
            return
        if self.suppress_event_printing and not force:
            return
        ...
        if token:
            ...
            self.console_print_token_counts[str(token)] = self.console_print_token_counts.get(str(token), 0) + 1
        self.console_print_counts[key] = self.console_print_counts.get(key, 0) + 1
    ...
    def _emit_recent_events(self) -> None:
        if not EVENT_PRINT_ENABLED:
            return
        events: List[Tuple[int, str, Dict[str, Any]]] = []
        for key, payload in self.console_event_log.items():
            ts = payload.get("time") if isinstance(payload, dict) else None
            if not self._event_print_within_age(ts):
                continue
            if isinstance(ts, (int, float)):
                events.append((int(ts), key, payload))
            else:
                events.append((0, key, payload))
        events.sort(key=lambda item: item[0])
        for _, key, payload in events:
            self._emit_console_event(key, payload, force=True)
    ...
    def process(self, candles: Sequence[Dict[str, float]]) -> None:
        ...
        self.console_printed_events.clear()
        self.console_printed_tokens.clear()
        self.console_print_counts.clear()
        self.console_print_token_counts.clear()
        self.suppress_event_printing = True

...
def evaluate_symbol_patterns(sym):
    """تجميع الأنماط مع جعل العائلات المؤسسية الأربع هي المرجع الحاكم، والقديم fallback فقط."""
    results = []
    ...
    has_core_bullish = any((r.get('direction') == 'UP' and r.get('pattern') in CORE_BULLISH_PATTERNS) for r in results)

    if not has_core_bullish:
        for fn in fallback_bullish_functions:
            res = _run_detector(fn)
            if res and res.get('direction') == 'UP':
                results.append(res)

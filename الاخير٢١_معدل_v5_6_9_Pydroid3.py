    ...
    def save_signal(self, signal_data):
        ...
        with sqlite3.connect(self.db_path, timeout=10) as conn:
            ...
            """, (
                ...
                signal_data.get('signal_quality_level', 'مرشح مبكر'),
...
def evaluate_symbol_patterns(sym):
    ...
    def _run_detector(fn):
        try:
            ...
            res['signal_quality_level'] = quality_level
...
def send_individual_signal(signal_data):
    ...
    message = (
        ...
        f"{primary_emoji} التصنيف: *{primary_name}* (`{primary_key}`) [{signal_data.get('signal_quality_level', 'مرشح مبكر')}]\n"
...
def generate_report(results):
    ...
    for r in results[:10]:
        ...
        lines.append(
            f"{primary_emoji} *{r['symbol']}*: {r['score']} | *{primary_name}* (`{primary_key}`) {multi_hint} [{r.get('signal_quality_level', 'مرشح مبكر')}]\n"
            f"   الاتجاه: {direction_ar}\n"
            f"   المرحلة: {signal_stage}\n"
            f"   السعر: {r['price']:.6f}\n"
            f"   وقف الخسارة: {stop_loss:.6f} | الهدف1: {tp1:.6f} | الهدف2: {tp2:.6f}\n"

...
def generate_report(results):
    ...
    lines = [
        ...
        "📌 الهدف: اكتشاف الارتفاع قبل حدوثه | اليومي=اتجاه | 4H=سياق | 15m=تهيئة | 5m=تفعيل\n"
    ]
    ...
    for r in bulls[:5]:
        ...
        now_state = f"الحالي: اتجاه {mtf.get('daily_bias','?')} مع تأكيد 4H={mtf.get('h4_bias','?')}"
        expected = "المتوقع: استمرار صعود تدريجي" if r.get('direction') == 'UP' else "المتوقع: ضغط هابط/تصحيح"
        lines.append(

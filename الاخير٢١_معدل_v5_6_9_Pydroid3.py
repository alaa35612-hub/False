...
def quick_filter():
    ...
    # أولوية سريعة للحالات position-led المبكرة حتى لا تفوت قبل deep-scan
    hinted = []
    non_hinted = []
    hint_checks = 0
    for symbol, volume, price_change_24h in pre_candidates:
        if hint_checks < 25:
            ...
            if pos and acc:
                ...
                if fast_position_led_hint(snapshot):
                    hinted.append((symbol, volume, price_change_24h))
                    continue
        non_hinted.append((symbol, volume, price_change_24h))
    pre_candidates = hinted + non_hinted

...
DEFAULT_CONFIG = {
    ...
    "SIGNAL_MIN_SCORE": 58,              # أقل درجة لقبول الإشارة المبكرة
...
class DiagnosticsLogger:
    ...
    def log_event(self, event_type, symbol, pattern, data=None):
        self._append({
            'type': 'event',
...
def classify_signal_stage(price_5m=0.0, price_15m=0.0, oi_5m=0.0, oi_15m=0.0, funding_rate=None, buy_ratio=0.0, score=0.0):
    ...
    if price_15m >= 6.5 or price_5m >= 3.2 or funding_abs >= 0.08:
        return 'LATE'
...
def pre_pump_early_filter(price_5m=0.0, price_15m=0.0, funding_rate=None, oi_15m=0.0):
    ...
    if price_15m >= 8.5 or price_5m >= 4.0:
        return False, 'late_price_extension'

    if funding_rate is not None and float(funding_rate) > 0.05 and price_15m >= 3.5:
        return False, 'crowded_positive_funding_after_move'

    if oi_15m >= 10.0 and price_15m >= 5.0:
...
def assess_signal_quality(symbol, base_score, pattern_data):
    ...
    try:
        ...
        if classification == 'POSITION_LED_SQUEEZE_BUILDUP' and stage == 'EARLY' and quality_score >= 58:
            quality_level = 'إشارة مؤسسية'
...
# =============================================================================
# تم حذف أنظمة التدريب/التعلم/المعايرة/المخاطر القديمة من مسار التشغيل
# =============================================================================
...
def detect_position_led_squeeze_buildup(symbol):
    ...
    try:
        ...
        early_core = (
            position_snapshot['ratio'] >= 1.20 and
            account_snapshot['ratio'] <= 1.05 and
            ls_ratio <= 0.95 and
            position_led_divergence >= 12
        )
        ...
        market_early_confirmation = (
            (price_change_5m >= 0.25 or price_change_15m >= 0.45) and
            (oi_change_5m >= 0.40 or oi_change_15m >= 1.00)
        )

        market_confirmed_confirmation = (
            (price_change_5m >= 0.70 or price_change_15m >= 1.00) and
            (oi_change_5m >= 0.80 or oi_change_15m >= 1.50)
        )

        context_ok = (
            abs(basis) <= 0.55 and
            abs(funding_rate) <= 0.12
        )
        ...
        if not early_core:
            return log_rejection(
                ...
                'early_core_not_ready',
                position_ratio=position_snapshot['ratio'],
                account_ratio=account_snapshot['ratio'],
                ls_ratio=ls_ratio,
                position_led_divergence=position_led_divergence
            )

        if not market_early_confirmation:
            return log_rejection(
                symbol,
                'POSITION_LED_SQUEEZE_BUILDUP',
                'early_market_confirmation_missing',
        ...
        if not flow_support:
            return log_rejection(
                symbol,
                'POSITION_LED_SQUEEZE_BUILDUP',
                'flow_support_missing',
                recent_buy=recent_buy,
                funding_trend=funding_trend,
                buy_accel=buy_accel
            )

        setup_strength = 'EARLY'
        if confirmed_core and market_confirmed_confirmation:
            setup_strength = 'CONFIRMED'
        ...
        base_score = 46 if setup_strength == 'EARLY' else 58
        score = int(
            base_score +
            weighted_feature_score(feature_scores) * 34 +
            oi_nv_score * 8 +
            compute_four_h_context_bonus(symbol) * 10
        )

        if setup_strength == 'EARLY':
            score = min(score, 82)
        reasons = [
        ...
        if setup_strength == 'EARLY':
            reasons.insert(0, "بصمة مبكرة: Position-led squeeze buildup قبل اكتمال الاندفاع")
        else:
            reasons.insert(0, "بصمة مؤكدة: Position-led squeeze buildup مع تأكيد سعري وOI")
        ...
        if setup_strength == 'EARLY':
            result['signal_stage'] = 'EARLY'
        elif result.get('signal_stage') in (None, '', 'SETUP', 'ARMED'):
            result['signal_stage'] = 'CONFIRMED'
...
class SignalDatabase:
    ...
    def save_signal(self, signal_data):
        ...
        with sqlite3.connect(self.db_path, timeout=10) as conn:
            self._ensure_signal_schema(conn)
            conn.execute("""
                INSERT INTO signals (timestamp, symbol, pattern, score, price, direction, reasons, signal_quality_tier, signal_stage, classification, trigger_high, trigger_low)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
...
# =============================================================================
# تم حذف نظام المعايرة القديم بالكامل
# =============================================================================
...
def build_mtf_context(symbol):
    ...
    if daily_trend_pct >= 1.2:
        daily_bias = 'UP'
        daily_reason = 'اتجاه يومي صاعد واضح مع زخم أعلى من الحد الأدنى'
    elif daily_trend_pct <= -1.2:
        daily_bias = 'DOWN'
        daily_reason = 'اتجاه يومي هابط واضح وضغط بيعي مستمر'
    else:
        daily_bias = 'NEUTRAL'
        daily_reason = 'اتجاه يومي محايد/متذبذب بدون انحياز قوي'

    if h4_trend_pct >= 0.6:
        h4_bias = 'UP'
        h4_reason = '4H يؤكد الميل الصاعد مع تسارع متوسط'
    elif h4_trend_pct <= -0.6:
        h4_bias = 'DOWN'
        h4_reason = '4H يؤكد الميل الهابط مع تسارع متوسط'
    else:
        h4_bias = 'NEUTRAL'
        h4_reason = '4H حيادي ولم يحسم التأكيد بعد'

    zone = 'NEUTRAL'
    zone_reason = 'لا توجد منطقة اهتمام واضحة على 15m'
    if abs(m15_move_pct) >= 0.5:
        if m15_move_pct > 0:
            zone = 'BULLISH_INTEREST'
            zone_reason = '15m يُظهر تهيئة صاعدة/Build-up في منطقة اهتمام'
        else:
            zone = 'BEARISH_INTEREST'
            zone_reason = '15m يُظهر ضعفًا/فشلًا في منطقة اهتمام هابطة'

    entry = 'WAIT'
    entry_reason = '5m لم يعطِ تفعيل دخول واضح'
    if m5_move_pct >= 0.20:
        entry = 'LONG_TRIGGER'
        entry_reason = '5m يعطي تفعيل دخول شراء مبكر'
    elif m5_move_pct <= -0.20:
        entry = 'SHORT_TRIGGER'
        entry_reason = '5m يعطي تفعيل دخول بيع/هبوط'
    ...
    return {
        ...
        'h4_reason': h4_reason,
        'm15_reason': zone_reason,
        'm5_reason': entry_reason,
    }
...
def is_mtf_aligned(direction, mtf):
    ...
    m15_zone = mtf.get('m15_zone', 'NEUTRAL')
    m5_entry = mtf.get('m5_entry', 'WAIT')

    if direction == 'UP':
        if daily == 'DOWN':
            return False, 'daily_bias_down'
        if h4 == 'DOWN':
            return False, 'h4_bias_down'
        if m15_zone == 'BEARISH_INTEREST':
            return False, 'm15_bearish_interest'
        if m5_entry == 'SHORT_TRIGGER':
            return False, 'm5_short_trigger'
        if daily == 'UP' and h4 == 'UP' and m15_zone in {'BULLISH_INTEREST', 'NEUTRAL'}:
            return True, 'full_up_alignment'
        return True, 'up_partial_alignment'

    if direction == 'DOWN':
        if daily == 'UP':
            return False, 'daily_bias_up'
        if h4 == 'UP':
            return False, 'h4_bias_up'
        if m15_zone == 'BULLISH_INTEREST':
            return False, 'm15_bullish_interest'
        if m5_entry == 'LONG_TRIGGER':
            return False, 'm5_long_trigger'
        if daily == 'DOWN' and h4 == 'DOWN' and m15_zone in {'BEARISH_INTEREST', 'NEUTRAL'}:
            return True, 'full_down_alignment'
        return True, 'down_partial_alignment'

    return False, 'unknown_direction'


def evaluate_symbol_patterns(sym):
    """تشغيل فعلي محصور في العائلات الأربع + ترشيح هرمي احتياطي لمنع ضياع الحالات المبكرة."""
    results = []
    ...
    def _run_detector(fn):
        try:
            ...
            if not aligned:
                return log_rejection(sym, res.get('pattern', 'UNKNOWN'), f'mtf_reject:{mtf_reason}', mtf=mtf)

            res['mtf_context'] = mtf
            res['daily_view'] = mtf.get('daily_reason', '')
            res['h4_view'] = mtf.get('h4_reason', '')
            res['m15_view'] = mtf.get('m15_reason', '')
            res['m5_view'] = mtf.get('m5_reason', '')
            res.setdefault('reasons', []).append(
                f"تسلسل MTF: 1D={mtf.get('daily_bias')} | 4H={mtf.get('h4_bias')} | 15m={mtf.get('m15_zone')} | 5m={mtf.get('m5_entry')}"
            )

            if mtf.get('daily_bias') == 'UP':
                res['score'] = min(100, float(res.get('score', 0)) + 4)
            if mtf.get('h4_bias') == 'UP':
                res['score'] = min(100, float(res.get('score', 0)) + 3)
            if mtf.get('m5_entry') == 'LONG_TRIGGER':
                res['score'] = min(100, float(res.get('score', 0)) + 2)

            quality_score, quality_level, quality_reasons = assess_signal_quality(res['symbol'], res['score'], res)
            res['score'] = max(float(res.get('score', 0)), float(quality_score))
    ...
    # fallback هرمي: إذا لم تنجح detectors رغم وجود توافق MTF واضح، أنشئ مرشح مبكر منطقي
    if not results:
        try:
            if mtf.get('daily_bias') in {'UP', 'NEUTRAL'} and mtf.get('h4_bias') in {'UP', 'NEUTRAL'} and mtf.get('m15_zone') == 'BULLISH_INTEREST' and mtf.get('m5_entry') != 'SHORT_TRIGGER':
                pos = get_top_position_snapshot(sym, period='5m')
                acc = get_top_account_snapshot(sym, period='5m')
                crowd = get_global_long_short_snapshot(sym, period='5m')
                ctx = get_short_term_context(sym, interval='5m', limit=12)
                if pos and acc and ctx:
                    ls_ratio = float((crowd or {}).get('ratio', acc.get('ratio', 1.0)) or acc.get('ratio', 1.0))
                    features = {
                        'position_ratio': float(pos.get('ratio', 0.0) or 0.0),
                        'account_ratio': float(acc.get('ratio', 0.0) or 0.0),
                        'ls_ratio': ls_ratio,
                        'top_position_long_pct': float(pos.get('long_pct', 50.0) or 50.0),
                        'top_account_long_pct': float(acc.get('long_pct', 50.0) or 50.0),
                        'oi_change_15m': float(ctx.get('oi_15m', 0.0) or 0.0),
                        'price_change_15m': float(ctx.get('price_15m', 0.0) or 0.0),
                        'price_change_5m': float(ctx.get('price_5m', 0.0) or 0.0),
                        'funding_rate': float(ctx.get('funding', 0.0) or 0.0),
                        'basis': float(ctx.get('basis', 0.0) or 0.0),
                        'taker_buy_ratio_recent': float(ctx.get('recent_buy_ratio', 0.0) or 0.0),
                        'trade_count_zscore': 0.0,
                    }
                    classification, decisive_feature = classify_bullish_signal(features)
                    if classification in CORE_BULLISH_PATTERNS:
                        fallback_score = 56.0
                        if mtf.get('daily_bias') == 'UP':
                            fallback_score += 4
                        if mtf.get('h4_bias') == 'UP':
                        ...
                        fallback = {
                            'symbol': sym,
                            'score': min(100.0, fallback_score),
                            'pattern': classification,
                            'classification': classification,
                            'direction': 'UP',
                            'signal_stage': 'EARLY',
                            'decisive_feature': decisive_feature,
                            'price': float(ctx.get('last_close', 0.0) or 0.0),
                            'feature_scores': {
                                'oi_regime': round(min(1.0, max(0.0, features['oi_change_15m'] / 3.0)), 3),
                                'taker_aggression': round(min(1.0, max(0.0, features['taker_buy_ratio_recent'])), 3),
                            },
                            'reasons': [
                                'توافق هرمي واضح بين 1D و4H و15m قبل الدخول',
                                f"15m في منطقة اهتمام صاعدة ({mtf.get('m15_move_pct', 0):+.2f}%)",
                                f"5m حالة التفعيل: {mtf.get('m5_entry')}",
                                'الترشيح مبني على حالة مؤسسية من العائلات الأربع وليس شرطًا منفردًا',
                            ],
                            'timestamp': datetime.now().isoformat(),
                            'mtf_context': mtf,
                            'daily_view': mtf.get('daily_reason', ''),
                            'h4_view': mtf.get('h4_reason', ''),
                            'm15_view': mtf.get('m15_reason', ''),
                            'm5_view': mtf.get('m5_reason', ''),
                        }
                        q_score, q_level, q_reasons = assess_signal_quality(sym, fallback['score'], fallback)
                        fallback['score'] = max(fallback['score'], q_score)
                        fallback['signal_quality_tier'] = q_level
                        for rr in q_reasons:
                            if rr not in fallback['reasons']:
                                fallback['reasons'].append(rr)
                        fallback = validate_and_finalize_signal(fallback)
                        if fallback:
                            results.append(fallback)
        except Exception as e:
            diagnostics.log_error(sym, 'fallback_mtf_candidate', str(e))
...
def generate_report(results):
    if not results:
        print("⚠️ لا توجد إشارات مؤهلة بعد التسلسل الهرمي 1D→4H→15m→5m. راجع diagnostics.jsonl لمعرفة سبب الرفض لكل حالة.")
        return

    results.sort(key=lambda x: x['score'], reverse=True)
    bulls = sorted([r for r in results if r.get('direction') == 'UP'], key=lambda x: x.get('score', 0), reverse=True)
    bears = sorted([r for r in results if r.get('direction') == 'DOWN'], key=lambda x: x.get('score', 0), reverse=True)
    top_results = (bulls[:5] + bears[:5]) if (bulls or bears) else results[:10]

    lines = [
        "🚀 *تقرير البصمات المؤسسية المبكرة*\n",
        "📌 الهدف: اكتشاف الارتفاع قبل حدوثه | اليومي=اتجاه | 4H=تأكيد | 15m=منطقة اهتمام | 5m=دخول\n"
    ]

    if bulls:
        lines.append("🟢 *أفضل العملات المرشحة للصعود:*")
    for r in bulls[:5]:
        _, primary_name, primary_emoji = get_primary_display(r)
        mtf = r.get('mtf_context', {})
        reasons = "، ".join(r.get('reasons', [])[:6])
        stage = r.get('signal_stage', 'SETUP')
        lines.append(
            f"{primary_emoji} *{r['symbol']}*\n"
            f"- الاتجاه المتوقع: صعود\n"
            f"- التصنيف الأساسي: {primary_name}\n"
            f"- جودة الإشارة: {r.get('signal_quality_tier', 'مرشح مبكر')}\n"
            f"- 1D: {mtf.get('daily_bias','?')} ({mtf.get('daily_trend_pct',0):+.2f}%) — {r.get('daily_view','')}\n"
            f"- 4H: {mtf.get('h4_bias','?')} ({mtf.get('h4_trend_pct',0):+.2f}%) — {r.get('h4_view','')}\n"
            f"- 15m: {mtf.get('m15_zone','?')} ({mtf.get('m15_move_pct',0):+.2f}%) — {r.get('m15_view','')}\n"
            f"- 5m: {mtf.get('m5_entry','?')} ({mtf.get('m5_move_pct',0):+.2f}%) — {r.get('m5_view','')}\n"
            f"- العامل الحاسم: {r.get('decisive_feature','score')}\n"
            f"- لماذا تم ترشيحها: {reasons}\n"
            f"- لماذا ليست عشوائية: توافق هرمي 1D→4H→15m→5m + حالة مؤسسية\n"
            f"- حالة الدخول: {stage}\n"
        )
    ...
    for r in bears[:5]:
        ...
        lines.append(
            ...
            f"- العامل الحاسم: {r.get('decisive_feature','score')}\n"
            f"- لماذا تم ترشيحها: {reasons}\n"
            f"- لماذا ليست عشوائية: توافق هرمي 1D→4H→15m→5m + حالة موثقة\n"
            f"- حالة الدخول: {stage}\n"
        )
...
def main():
    print("="*70)
    print("الماسح المتوازي المتقدم - تحليل متعدد الفريمات".center(70))
    print("="*70)

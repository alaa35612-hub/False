...
def _set_rate_limits_from_cfg(cfg: Any) -> None:
    ...
    try:
        ...
        max_concurrent = 3
    try:
        requests_per_sec = float(getattr(cfg, "max_requests_per_sec", max_concurrent))
    except Exception:
        requests_per_sec = float(max_concurrent or 5.0)
    try:
    ...
    try:
        ...
        backoff_base = 0.5
    _GLOBAL_RATE_LIMITER = RateLimiter(
        max_per_sec= requests_per_sec if requests_per_sec > 0 else float(max_concurrent or 5.0),
...
class SmartMoneyAlgoProE5:
    ...
    def _register_box_event(self, box: Box, *, status: str = "active", event_time: Optional[int] = None) -> None:
        ...
        if key:
            ...
            tally[status_key] += 1
            # Keep touched IDs so the scanner can skip mitigated zones like the Pine indicator
            if status_key in {"touched", "retest"}:
                if key == "EXT_OB":
                    self._ext_touched_ids.add(id(box))
                elif key == "IDM_OB":
                    self._idm_touched_ids.add(id(box))
                elif key == "HIST_EXT_OB":
                    self._hist_ext_touched_ids.add(id(box))
                elif key == "HIST_IDM_OB":
                    self._hist_idm_touched_ids.add(id(box))
            self.console_event_log[key] = {
                "text": box.text,
    ...
    def _initialise_state(self) -> None:
        ...
        self._ext_touched_ids: set[int] = set()
        self._idm_touched_ids: set[int] = set()
...
def _box_is_touched(state: Any, box: Box) -> bool:
    ...
    try:
        if id(box) in getattr(state, "_ext_touched_ids", set()):
            return True
        if id(box) in getattr(state, "_idm_touched_ids", set()):
            return True
...
def _parse_args_android() -> Tuple[_CLISettings, argparse.Namespace]:
    ...
    p.add_argument("--max-requests-per-sec", type=float, default=8.0)

...
# =============================================================================
# الإعدادات العامة للماسح والتنبيهات (قابلة للتعديل بسرعة من هنا)
# -----------------------------------------------------------------------------
...
# DEBUG_MODE: يطبع سبب تجاهل الحدث (stale/invalid/toggle/duplicate).
...
ALERT_TOGGLES: Dict[str, bool] = {
    ...
    "bearish_internal_ob": True,
    "bullish_internal_ob": True,
    ...
    "bullish_fvg": True,
    "bearish_fvg": True,
    "bullish_fvg_break": True,
    "bearish_fvg_break": True,
    "bullish_mss": True,
    "bearish_mss": True,
    "bullish_bos": True,
    "bearish_bos": True,
    ...
    "inside_bar": True,
    "key_level": True,
    "high_liquidity_level": True,
    "low_liquidity_level": True,
    "high_liquidity_level_break": True,
    "low_liquidity_level_break": True,
    "golden_zone_created": True,
    "golden_zone_first_touch": True,
    "idm_ob_created": True,
    "ext_ob_created": True,
    "ext_ob_first_touch": True,
    "idm_ob_first_touch": True,
    "hist_ext_ob_first_touch": True,
    "hist_idm_ob_first_touch": True,
}
...
class AlertToggleConfig:
    ...
    bullish_mss: bool = True
    bearish_mss: bool = True
    bullish_bos: bool = True
    bearish_bos: bool = True
    outside_bar: bool = True
...
DEFAULT_ALERT_TOGGLES = AlertToggleConfig(**ALERT_TOGGLES)
...
class SmartMoneyAlgoProE5:
    ...
    ALERT_TITLE_TOGGLE_MAP = {
        ...
        "Bullish MSS": "bullish_mss",
        "Bearish MSS": "bearish_mss",
    ...
    ALERT_TITLE_EVENT_KEY_MAP = {
        "Bearish External OB": "ALERT_BEARISH_EXTERNAL_OB",
        "Bullish External OB": "ALERT_BULLISH_EXTERNAL_OB",
        "Bearish Internal OB": "ALERT_BEARISH_INTERNAL_OB",
        "Bullish Internal OB": "ALERT_BULLISH_INTERNAL_OB",
        "Bullish OB Break": "ALERT_BULLISH_OB_BREAK",
        "Bearish OB Break": "ALERT_BEARISH_OB_BREAK",
        "Bullish Sweep": "ALERT_BULLISH_SWEEP",
        ...
        "Bullish MSS": "ALERT_BULLISH_MSS",
        "Bearish MSS": "ALERT_BEARISH_MSS",
        "Bullish BOS": "ALERT_BULLISH_BOS",
        "Bearish BOS": "ALERT_BEARISH_BOS",
        "Outside Bar": "OUTSIDE_BAR",
        "Inside Bar": "INSIDE_BAR",
        "Key Level": "KEY_LEVEL",
        "High Liquidity Level": "ALERT_HIGH_LIQUIDITY_LEVEL",
        "Low Liquidity Level": "ALERT_LOW_LIQUIDITY_LEVEL",
        "High Liquidity Level Break": "ALERT_HIGH_LIQUIDITY_LEVEL_BREAK",
        "Low Liquidity Level Break": "ALERT_LOW_LIQUIDITY_LEVEL_BREAK",
        "Golden Zone Created": "GOLDEN_ZONE_CREATED",
        "Golden Zone Created (Untouched)": "GOLDEN_ZONE_CREATED",
        "Golden Zone First Touch": "GOLDEN_ZONE_FIRST_TOUCH",
        "IDM OB Zone Created": "IDM_OB_CREATED",
    ...
    def alertcondition(self, condition: bool, title: str, message: Optional[str] = None) -> None:
        ...
        if toggle_key and not getattr(self.alert_toggles, toggle_key, True):
            return None
        event_time = self.series.get_time()
        try:
            event_ts = int(event_time) if isinstance(event_time, (int, float)) else 0
        except Exception:
            event_ts = 0
        ...
        if not event_key:
            return None
        display = message or title
        direction = _resolve_direction(title, message)
        self.console_event_log[event_key] = {
            "text": title,
            "price": None,
            "time": event_ts,
            "time_display": format_timestamp(event_ts),
            "display": display,
            "direction": direction,
        }
        return None
    ...
    def _register_label_event(self, label: Label) -> None:
        ...
        if collapsed == "BOS":
            ...
            lowered = text.lower()
            if "outside bar" in lowered:
                key = "OUTSIDE_BAR"
            elif "inside bar" in lowered:
                key = "INSIDE_BAR"
            elif any(token in text for token in ("MDAY", "PWH", "PWL", "WO", "MO", "PMH", "PML", "Weekly", "Monthly", "Monday")):
                key = "KEY_LEVEL"
        if key:
            if key in ("BOS", "CHOCH"):
        ...
        if key:
            ...
            if status_key == "new":
                alert_titles = {
                    ...
                    "GOLDEN_ZONE": "Golden Zone Created (Untouched)",
                }
...
EVENT_DISPLAY_ORDER = [
    ...
    ("GOLDEN_ZONE_CREATED", "Golden Zone Created"),
    ("GOLDEN_ZONE_FIRST_TOUCH", "Golden Zone First Touch"),
    ("IDM_OB_CREATED", "IDM OB Created"),
    ("EXT_OB_CREATED", "EXT OB Created"),
    ("EXT_OB_FIRST_TOUCH", "EXT OB First Touch"),
    ("IDM_OB_FIRST_TOUCH", "IDM OB First Touch"),
    ("HIST_EXT_OB_FIRST_TOUCH", "Hist EXT OB First Touch"),
    ("HIST_IDM_OB_FIRST_TOUCH", "Hist IDM OB First Touch"),
    ("X", "X"),
    ...
    ("ALERT_BULLISH_MSS", "Bullish MSS"),
    ("ALERT_BEARISH_MSS", "Bearish MSS"),
    ("ALERT_BULLISH_BOS", "Bullish BOS"),
    ("ALERT_BEARISH_BOS", "Bearish BOS"),
...
EVENT_TOGGLE_KEY_MAP = {
    ...
    "ALERT_BULLISH_MSS": "bullish_mss",
    "ALERT_BEARISH_MSS": "bearish_mss",
    "ALERT_BULLISH_BOS": "bullish_bos",
    "ALERT_BEARISH_BOS": "bearish_bos",
...
def _is_price_inside_golden_zone(metrics: Dict[str, Any]) -> bool:
    """
    إرجاع True فقط إذا كان السعر الحالي داخل منطقة Golden Zone النشطة (إن وُجدت),
    وإلا تُرجِع False. تُستخدم هذه الدالة في المسح/الـ CLI لتصفية الأزواج.
    """
    try:
        latest = metrics.get("latest_events") or {}
    except Exception:
        return False
    ...
    try:
        price = float(metrics.get("current_price"))
    except (TypeError, ValueError):
        return False

    if math.isnan(price) or math.isnan(lower) or math.isnan(upper):
        return False
...
def _event_bars_ago_from_metrics(event_time: Any, metrics: Dict[str, Any], timeframe: str) -> Optional[int]:
    if not isinstance(event_time, (int, float)):
        return None
    latest_bar_time = metrics.get("latest_bar_time")
    if not isinstance(latest_bar_time, (int, float)):
        return None
    seconds = _parse_timeframe_to_seconds(timeframe, None)
    if seconds is None or seconds <= 0:
        return None
    diff_ms = int(latest_bar_time) - int(event_time)
    if diff_ms < 0:
        return 0
...
def _event_within_age(event_time: Any, timeframe: str, metrics: Dict[str, Any]) -> bool:
    ...
    if max_age is None or max_age <= 0:
        return True
    bars_ago = _event_bars_ago_from_metrics(event_time, metrics, timeframe)
    if bars_ago is None:
        return True
...
EVENT_ALERT_DEFINITIONS: Sequence[Tuple[str, str, str]] = (
    ("ALERT_BULLISH_EXTERNAL_OB", "Bullish External OB", "bullish_external_ob"),
    ("ALERT_BEARISH_EXTERNAL_OB", "Bearish External OB", "bearish_external_ob"),
    ("ALERT_BULLISH_INTERNAL_OB", "Bullish Internal OB", "bullish_internal_ob"),
    ("ALERT_BEARISH_INTERNAL_OB", "Bearish Internal OB", "bearish_internal_ob"),
    ("ALERT_BULLISH_OB_BREAK", "Bullish OB Break", "bullish_ob_break"),
    ("ALERT_BEARISH_OB_BREAK", "Bearish OB Break", "bearish_ob_break"),
    ("ALERT_BULLISH_SWEEP", "Bullish Sweep", "bullish_sweep"),
    ...
    ("ALERT_BULLISH_MSS", "Bullish MSS", "bullish_mss"),
    ("ALERT_BEARISH_MSS", "Bearish MSS", "bearish_mss"),
    ("ALERT_BULLISH_BOS", "Bullish BOS", "bullish_bos"),
    ("ALERT_BEARISH_BOS", "Bearish BOS", "bearish_bos"),
    ("OUTSIDE_BAR", "Outside Bar", "outside_bar"),
    ("INSIDE_BAR", "Inside Bar", "inside_bar"),
    ("KEY_LEVEL", "Key Level", "key_level"),
    ("ALERT_HIGH_LIQUIDITY_LEVEL", "High Liquidity Level", "high_liquidity_level"),
    ("ALERT_LOW_LIQUIDITY_LEVEL", "Low Liquidity Level", "low_liquidity_level"),
    ("ALERT_HIGH_LIQUIDITY_LEVEL_BREAK", "High Liquidity Level Break", "high_liquidity_level_break"),
    ("ALERT_LOW_LIQUIDITY_LEVEL_BREAK", "Low Liquidity Level Break", "low_liquidity_level_break"),
    ("GOLDEN_ZONE_CREATED", "Golden Zone Created", "golden_zone_created"),
    ("GOLDEN_ZONE_FIRST_TOUCH", "Golden Zone First Touch", "golden_zone_first_touch"),
    ("IDM_OB_CREATED", "IDM OB Created", "idm_ob_created"),
    ("EXT_OB_CREATED", "EXT OB Created", "ext_ob_created"),
    ("EXT_OB_FIRST_TOUCH", "EXT OB First Touch", "ext_ob_first_touch"),
    ("IDM_OB_FIRST_TOUCH", "IDM OB First Touch", "idm_ob_first_touch"),
    ("HIST_EXT_OB_FIRST_TOUCH", "Hist EXT OB First Touch", "hist_ext_ob_first_touch"),
    ("HIST_IDM_OB_FIRST_TOUCH", "Hist IDM OB First Touch", "hist_idm_ob_first_touch"),
)
...
def _build_golden_zone_alert_line(symbol: str, timeframe: str, event: Dict[str, Any], metrics: Dict[str, Any]) -> str:
    ...
    price_text = f"{format_price(lower)} → {format_price(upper)}"
    try:
        close_val = metrics.get("current_price")
        close_text = format_price(float(close_val)) if close_val is not None else "—"
    except Exception:
        close_text = "—"
    ts = event.get("time")
    ts_display = format_timestamp(ts) if isinstance(ts, (int, float)) else "—"
    return (
        f"{symbol} {timeframe} | Golden Zone First Touch | "
        f"Range: {price_text} | Close: {close_text} | Time: {ts_display}"
    )


def _format_event_display(event: Dict[str, Any]) -> str:
    display_text = event.get("display")
    if display_text is None:
        price = event.get("price")
...
def _build_event_alert_lines(symbol: str, timeframe: str, metrics: Dict[str, Any], *, use_memory: bool = True) -> List[str]:
    ...
    for event_key, label, toggle in EVENT_ALERT_DEFINITIONS:
        if toggle and not getattr(DEFAULT_ALERT_TOGGLES, toggle, True):
            if (DEBUG_LOGS or DEBUG_MODE) and event_key in latest_events:
        ...
        try:
            ...
            close_text = "—"
        time_text = format_timestamp(event_ts)
        lines.append(
            f"{symbol} {timeframe} | {label} | Range: {price_text} | Close: {close_text} | UTC: {time_text}"
        )
    return lines
...
def _detect_golden_zone_first_touch(symbol: str, timeframe: str, metrics: Dict[str, Any]) -> Optional[str]:
    ...
    if not isinstance(latest_events, dict):
        return None
    event = latest_events.get("GOLDEN_ZONE")
    if not isinstance(event, dict):
        return None
    if ALERT_ONLY_FOR_GOLDEN_ZONE and str(event.get("text", "")).strip() != "Golden zone":
        return None
    if event.get("status") != "touched":
        return None
    event_time = event.get("time")
    if not _golden_zone_event_within_age(event_time, timeframe, metrics):
        if DEBUG_LOGS:
            print(
                f"[DBG] Skipping stale Golden Zone touch for {symbol} {timeframe} (event time {event_time})",
                file=sys.stderr,
            )
        return None
    try:
        zone_ts = int(event_time) if isinstance(event_time, (int, float)) else None
    except Exception:
        zone_ts = None
    if zone_ts is None:
        return None
    alert_key = (symbol.upper(), timeframe, zone_ts)
...
def in_fvg(state: Any) -> Tuple[bool, Optional[str]]:
    price = getattr(getattr(state, "series", None), "get", lambda *_: None)("close")
    if not isinstance(price, (int, float)) or math.isnan(price):
        return False, None
...
def run_golden_zone_first_touch_scan(
    symbols: Optional[List[str]] = None,
    *,
    timeframes: Optional[Sequence[str]] = None,
    candle_limit: Optional[int] = None,
) -> List[str]:
    """مسح جميع الإطارات الزمنية المحددة لرصد أول ملامسة لـ Golden Zone وإرسال تنبيه واحد لكل منطقة."""

    alerts: List[str] = []
    tf_list = list(timeframes) if timeframes else list(SCAN_TIMEFRAMES)
    if not tf_list:
        return alerts

    for tf in tf_list:
        window = MAX_EVENT_AGE_BARS.get(tf, 0)
        indicator_inputs = IndicatorInputs()
        if window and window > 0:
            try:
                indicator_inputs.console.max_age_bars = max(
                    indicator_inputs.console.max_age_bars, int(window)
                )
            except Exception:
                indicator_inputs.console.max_age_bars = max(
                    indicator_inputs.console.max_age_bars, 1
                )
        runtime, summaries = scan_binance(
            tf,
            candle_limit or SCAN_CANDLE_LIMIT,
            symbols,
            concurrency=3,
            tracer=None,
            ...
            max_symbols=EDITOR_AUTORUN_DEFAULTS.max_symbols,
            send_alerts=False,
        )
        for row in summaries:
            sym = row.get("symbol", "")
            metrics = row.get("metrics", {})
            alert_line = _detect_golden_zone_first_touch(sym, tf, metrics)
            if alert_line:
                alerts.append(alert_line)
                if DEBUG_LOGS:
                    print(f"[DBG] Golden Zone alert queued for {sym} {tf}", flush=True)

    if alerts:
        _send_telegram_lines(alerts)
        if DEBUG_LOGS:
            print(f"[DBG] Sent {len(alerts)} Golden Zone alerts", flush=True)
    elif DEBUG_LOGS:
        print("[DBG] No new Golden Zone first-touch events detected", flush=True)
    return alerts
...
def _get_secret(name: str) -> Optional[str]:
    if name == "TELEGRAM_BOT_TOKEN" and TELEGRAM_BOT_TOKEN:
        return TELEGRAM_BOT_TOKEN
    if name == "TELEGRAM_CHAT_ID" and TELEGRAM_CHAT_ID:
...
def _send_telegram_lines(lines: List[str]) -> None:
    ...
    try:
        ...
        if DEBUG_LOGS:
            print("[DBG] Failed to send Telegram alert", file=sys.stderr)
...
def _parse_args_android() -> Tuple[_CLISettings, argparse.Namespace]:
    ...
    p.add_argument("--golden-scan", action="store_true", help="تشغيل ماسح اللمسة الأولى للـ Golden Zone")
    # misc
...
def _should_route_android(argv: List[str]) -> bool:
    knobs = {
        ...
        "--min-change","--min-volume","--max-scan","--allow-meme","--exclude-symbols","--exclude-patterns","--include-only","--golden-scan",
...
_LAST_BUCKETS = {
    ...
    "Golden Zone Created": ["golden zone created"],
    "Golden Zone First Touch": ["golden zone first touch"],
    "IDM": ["idm @", " i d m "],
    "EXT OB": ["ext ob", "ext_ob", "ext  ob"],
    "IDM OB": ["idm ob"],
    "Hist IDM OB": ["hist idm ob"],
    "Hist EXT OB": ["hist ext ob"],
    "IDM OB Created": ["idm ob zone created", "idm ob created"],
    "EXT OB Created": ["ext ob zone created", "ext ob created"],
    "EXT OB First Touch": ["ext ob first touch"],
    "IDM OB First Touch": ["idm ob first touch"],
    "Hist EXT OB First Touch": ["hist ext ob first touch"],
    "Hist IDM OB First Touch": ["hist idm ob first touch"],
    ...
    "Bullish External OB": ["bullish external ob"],
    "Bearish Internal OB": ["bearish internal ob"],
    "Bullish Internal OB": ["bullish internal ob"],
    ...
    "Bearish FVG": ["bearish fvg"],
    "Bullish FVG Break": ["bullish fvg break"],
    "Bearish FVG Break": ["bearish fvg break"],
    "High Liquidity Level": ["high liquidity level"],
    "Low Liquidity Level": ["low liquidity level"],
    "High Liquidity Level Break": ["high liquidity level break"],
    "Low Liquidity Level Break": ["low liquidity level break"],
}
...
_METRIC_MAP = {
    ...
    "Golden Zone Created": ["GOLDEN_ZONE_CREATED", "golden_zone_created"],
    "Golden Zone First Touch": ["GOLDEN_ZONE_FIRST_TOUCH", "golden_zone_first_touch"],
    "IDM": ["IDM", "idm"],
    "EXT OB": ["EXT_OB", "ext_ob", "EXT-OB"],
    "IDM OB": ["IDM_OB", "idm_ob"],
    "Hist IDM OB": ["HIST_IDM_OB", "hist_idm_ob"],
    "Hist EXT OB": ["HIST_EXT_OB", "hist_ext_ob"],
    "IDM OB Created": ["IDM_OB_CREATED", "idm_ob_created"],
    "EXT OB Created": ["EXT_OB_CREATED", "ext_ob_created"],
    "EXT OB First Touch": ["EXT_OB_FIRST_TOUCH", "ext_ob_first_touch"],
    "IDM OB First Touch": ["IDM_OB_FIRST_TOUCH", "idm_ob_first_touch"],
    "Hist EXT OB First Touch": ["HIST_EXT_OB_FIRST_TOUCH", "hist_ext_ob_first_touch"],
    "Hist IDM OB First Touch": ["HIST_IDM_OB_FIRST_TOUCH", "hist_idm_ob_first_touch"],
    "الدوائر الحمراء": ["RED_CIRCLE", "red_circle", "RED_CIRCLES"],
    "الدوائر الخضراء": ["GREEN_CIRCLE", "green_circle", "GREEN_CIRCLES"],
    "Bearish External OB": ["ALERT_BEARISH_EXTERNAL_OB", "BEARISH_EXTERNAL_OB"],
    "Bullish External OB": ["ALERT_BULLISH_EXTERNAL_OB", "BULLISH_EXTERNAL_OB"],
    "Bearish Internal OB": ["ALERT_BEARISH_INTERNAL_OB", "BEARISH_INTERNAL_OB"],
    "Bullish Internal OB": ["ALERT_BULLISH_INTERNAL_OB", "BULLISH_INTERNAL_OB"],
    "Bullish OB Break": ["ALERT_BULLISH_OB_BREAK", "BULLISH_OB_BREAK"],
    "Bearish OB Break": ["ALERT_BEARISH_OB_BREAK", "BEARISH_OB_BREAK"],
    "Bullish Sweep": ["ALERT_BULLISH_SWEEP", "BULLISH_SWEEP"],
    ...
    "Low Liquidity Level": ["ALERT_LOW_LIQUIDITY_LEVEL", "LOW_LIQUIDITY_LEVEL"],
    "High Liquidity Level Break": ["ALERT_HIGH_LIQUIDITY_LEVEL_BREAK", "HIGH_LIQUIDITY_LEVEL_BREAK"],
    "Low Liquidity Level Break": ["ALERT_LOW_LIQUIDITY_LEVEL_BREAK", "LOW_LIQUIDITY_LEVEL_BREAK"],
}
...
def __router_main__():
    ...
    argv = sys.argv[1:]
    try:
        use_cli = _should_route_android(argv)
    except Exception:
        use_cli = False
    if (not argv) and ('_android_cli_entry' in globals()):
        ...
        if defaults.height_threshold is not None:
            argv += ["--min-change", str(defaults.height_threshold)]
        if defaults.height_candle_window is not None:
            argv += ["--height-candles", str(defaults.height_candle_window)]
        if defaults.height_scope:
            argv += ["--height-scope", str(defaults.height_scope)]
        if defaults.height_metric:
            argv += ["--height-metric", str(defaults.height_metric)]
        if defaults.continuous_scan:
            argv.append("--continuous")
        if defaults.scan_interval > 0:
            argv += ["--continuous-interval", str(defaults.scan_interval)]
...
def _android_cli_entry() -> int:
    ...
    try:
        while True:
            ...
            if getattr(args, "golden_scan", False):
                try:
            ...
            telegram_lines: List[str] = []
            for timeframe in active_timeframes:
                event_window = MAX_EVENT_AGE_BARS.get(timeframe, recent_window)
                if event_window and event_window > 0:
                    inputs.console.max_age_bars = max(1, int(event_window))
                for i, sym in enumerate(symbols, 1):
                    try:
    ...
    p.add_argument("--golden-scan", action="store_true", help="تشغيل ماسح اللمسة الأولى للـ Golden Zone عبر جميع الفريمات")
...
if __name__ == "__main__":
    argv = sys.argv[1:]
    if (not argv) or _should_route_android(argv):
        sys.exit(__router_main__())
    # تشغيل تلقائي من المحرر بالقيم الإفتراضية أعلاه عند استخدام وضع الباكتيست/الاستراتيجيات
    _main(argv)
# ============================ End of Integration ============================
